(() => {
  ...
  function award({unitId,phase,itemId,correct,wasCompleted,unitComplete}) { if(!correct) return; const before=rank(); const reward=`${unitId}.${phase}.${itemId}`; let gained=0;
  ...
  function catalog() { const facts=FACTS[grade]||{}; const items=[]; Object.entries(facts).forEach(([id,fact])=> { const got=state.discoveries[`major.${id}`];items.push(`<article class="discovery-card ${got?"found":"hidden"}"><span>${got?"★":"？"}</span><b>${got?esc(got.title):"？？？"}</b><small>${got?esc(fact):"研究を進めると見つかるよ"}</small></article>`); }); return `<section class="catalog-page"><button class="text-button" data-home>単元一覧へ</button><p class="eyebrow">DISCOVERY BOOK</p><h1>発見図鑑　${info().discoveries} / ${info().total}</h1><p>見つけた大発見は、いつでもここで確かめられるよ。</p><div class="discovery-grid">${items.join("")}</div></section>`; }
  window.ScienceGame={award,panel,catalog};
})();
