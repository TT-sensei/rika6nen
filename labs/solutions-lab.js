(() => {
  ...
  function mount(root,{manifest}){
    ...
    root.addEventListener('click',e=>{
      ...
      if(e.target.matches('[data-unknown]')){s.solution=e.target.dataset.unknown;s.pipette=0;s.evap=initial().evap;s.gas=initial().gas;s.metal=initial().metal;draw();guide("次の実験を選び、証拠を集めよう。");}
      ...
      if(e.target.matches('[data-result-reset]')){save({version:1,records:[],evidence:[]});draw();guide("結果表と証拠カードを0件に戻しました。新しい実験を始めよう。");}
    });
