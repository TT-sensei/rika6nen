(() => {
  ...
  const initial = () => ({ covered:false, jarX:700, bottomGap:false, topGap:false, lit:false, time:0, oxygen:21, co2:0.04, beforeOxygen:21, beforeCo2:0.04, flow:true, inside:false, speed:1, sample:"after", collected:false, lime:false, oxygenTest:false, shaken:false });
  ...
  function scene(s, m) {
    ...
    return `<svg class="burning-svg" viewBox="0 0 900 520" role="img" aria-label="燃焼実験台"><defs><linearGradient id="jarAir" x1="0" x2="0" y1="0" y2="1"><stop stop-color="#d8f0f4" stop-opacity=".8"/><stop offset="1" stop-color="#aad5df" stop-opacity=".45"/></linearGradient><filter id="glow"><feGaussianBlur stdDeviation="7" result="b"/><feMerge><feMergeNode in="b"/><feMergeNode in="SourceGraphic"/></feMerge></filter></defs><rect width="900" height="520" fill="#f1f7f5"/><text x="32" y="42" class="scene-title">燃焼実験台</text><path d="M120 410h660" stroke="#805a42" stroke-width="14"/><path d="M155 410v55M745 410v55" stroke="#805a42" stroke-width="12"/><text x="132" y="495" class="scene-note">集気びんをドラッグして、ろうそくにかぶせる</text><g class="candle"><rect x="418" y="285" width="64" height="125" rx="13" fill="#fff7df" stroke="#a78f66" stroke-width="4"/><path d="M450 285v-24" stroke="#4b4037" stroke-width="5"/>${flame}</g>${jar}${bottom}${top}${airflow(s)}${gasParticles(s)}<g data-action="lighter" class="lighter"><circle cx="727" cy="260" r="28" fill="#fff2c5" stroke="#d77a46" stroke-width="5"/><path d="M727 278v-20" stroke="#87452d" stroke-width="7"/><path d="M727 244c-10 9 8 14 0 23 18-9 8-19 0-23z" fill="#ff9c28"/><rect x="705" y="305" width="44" height="90" rx="12" fill="#d77a46" stroke="#87452d" stroke-width="4"/><text x="727" y="430" text-anchor="middle" class="scene-label">ここをタップして点火</text></g><g data-action="cover" class="cover-handle"><rect x="270" y="55" width="360" height="48" rx="18" fill="#dbe8e8" stroke="#518696" stroke-width="6"/><path d="M420 55h60" stroke="#6d8991" stroke-width="14" stroke-linecap="round"/><text x="450" y="86" text-anchor="middle" class="tap-label">ふた：${s.covered ? "かぶせています" : "ここをタップ"}</text></g><text x="450" y="475" text-anchor="middle" class="scene-caption">${s.lit ? (m.flame < .18 ? "炎が消えそうです" : "燃焼中") : (s.time ? "炎が消えました。条件を変えて再実験できます" : "まず、右の点火器具をタップ")}</text></svg>`;
  }
